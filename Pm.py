...
class Pm:
    ...
    def getFriendInfo(self):
        return self.__operator.get_friend_info(self.tuin)
    ...
    def game(self, msg):
        # 阿瓦隆投票
        match = re.match(ur'^\s*y\s*$', msg.content)  # 同意
        if match:
            f = open('avalon.txt','r')
            ...
            nCount = voteInfo[2][:-1]
            names = voteInfo[3:]
            player = self.getFriendInfo()+'\n'
            f.close()
            if player in names:
                names.remove(player)
                restNum = str(int(restNum)-1)
                f = open('avalon.txt','w')
                f.write(result+'\n')
                f.write(restNum+'\n')
                f.write(nCount+'\n')
                for x in names:
                    f.write(x)
                f.close()
                self.reply("投票成功！已投同意票")
                return True
            
        match = re.match(ur'^\s*n\s*$', msg.content)  # 反对
        if match:
            f = open('avalon.txt','r')
            voteInfo = f.readlines()
            result = voteInfo[0][:-1]
