...
from Configs import *
from Msg import *
from plugin import shuishiwodi, shuishiwodiStartStatus, avalon, avalonStartStatus
...
class Group:
    ...
    def get_menber_info(self, tuin):
        return self.__operator.get_friend_info(tuin)
    ...
    def reply_sess(self, tuin, reply_content, service_type=0):
        self.msg_id += 1
        self.__operator.send_buddy_msg(tuin, reply_content, self.msg_id)
    ...
    def ask(self, msg):
        match = re.match(ur'^(ask|问|小c) (\w+|[\u4e00-\u9fa5]+)', msg.content)
        if match:
            # logging.info("问答测试...")
            print msg.content
            command = match.group(1)
            ...
            logging.info(msg.content)
            # print info
            if command == 'ask' or command == u'问' or command == u'小c':
    ...
    def game(self, msg):
        ...
        if match:
            ...
            if args1 and u'谁是卧底' in args1:
                ...
                return True
            elif args1 and u'阿瓦隆' in args1:
                self.__game_handler = avalon(avalonStartStatus(), self)
                self.__game_handler.run(msg)
                return True
            return True
        # 没有处理程序时退出
        ...
        # 阿瓦隆的处理程序
        if isinstance(self.__game_handler, avalon):
            if self.__game_handler.status not in ['StartStatus', 'EndStatus']:
                self.__game_handler.run(msg)
                return True  # 游戏期间屏蔽其他处理过程
        return False
