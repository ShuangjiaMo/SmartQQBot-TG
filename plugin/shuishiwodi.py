...
class AssignRolesStatus(StatusHandler):
    ...
    def __assignRoles(self):
        ...
        # 私聊玩家，通知词语
        for x in game.playerList:
            game.writePrivate(x.uin, u'[谁是卧底][%s]您本局的词语是：%s' % (x.id, x.word))
        # 进入发言阶段
...
class SpeechStatus(StatusHandler):
    ...
    def handle(self, game, msgDto):
        ...
        matchSuccess = re.match(ur'^\s*开始投票\s*$', msgDto.content)
        if not matchSuccess:
            return False
        uin = msgDto.send_uin
    ...
    def __first(self):
        ...
        playerInfo = lst[i]
        game.writePublic(u"发言阶段，请从%d号玩家[%s]开始，依次发言。\n（回复“开始投票”进行投票）" % (playerInfo.id, playerInfo.name))
        return

    def __startNotifyThread(self, timeout=600):
        def process(statusHandle, game, timeout):
            while statusHandle == game.statusHandle and timeout > 0:
                time.sleep(10)
                timeout -= 10
...
class VoteStatus(StatusHandler):
    ...
    def __first(self):
        ...
        game.writePublic(u"发言结束，下面开始投票。")
        return
